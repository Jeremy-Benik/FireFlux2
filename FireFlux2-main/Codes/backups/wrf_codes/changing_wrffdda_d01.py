...
import numpy as np
import pandas as pd
from wrf import vinterp, getvar, destagger #These are used to read in the wrf variables and to interpolate the variables at a list of heights. 
# %% Calling in the files and defining them
wrfin_ideal = nc.Dataset('/home/jbenik/FirefluxII/Codes_and_Data/Data/wrf_files/wrfinput_d01_ideal', 'r', format='NETCDF4')
wrfin_real = nc.Dataset('/home/jbenik/FirefluxII/Codes_and_Data/Data/wrf_files/wrfin_d01_real', 'r', format='NETCDF4')
...
height_ideal /= 1000
height_ideal = height_ideal[0, :, 0, 0]
...
wrffdda_u_old_height = vinterp(wrfin_real, U_old, 'ght_agl', height_real, extrapolate = True)
wrffdda_v_old_height = vinterp(wrfin_real, V_old, 'ght_agl', height_real, extrapolate = True)
wrffdda_t_old_height = vinterp(wrfin_real, T_old, 'ght_agl', height_real, extrapolate = True)
wrffdda_q_old_height = vinterp(wrfin_real, Q_old, 'ght_agl', height_real, extrapolate = True)
wrffdda_u_new_height = vinterp(wrfin_real, U_new, 'ght_agl', height_real, extrapolate = True)
wrffdda_v_new_height = vinterp(wrfin_real, V_new, 'ght_agl', height_real, extrapolate = True)
wrffdda_t_new_height = vinterp(wrfin_real, T_new, 'ght_agl', height_real, extrapolate = True)
wrffdda_q_new_height = vinterp(wrfin_real, Q_new, 'ght_agl', height_real, extrapolate = True)
print("Interpolating the heights from the wrfin_ideal")
wrfin_u = vinterp(wrfin_ideal, wrffdda_u_old_height, 'ght_agl', height_ideal, extrapolate = True)
wrfin_v = vinterp(wrfin_ideal, wrffdda_v_old_height, 'ght_agl', height_ideal, extrapolate = True)
wrfin_q = vinterp(wrfin_ideal, wrffdda_q_old_height, 'ght_agl', height_ideal, extrapolate = True)
wrfin_t = vinterp(wrfin_ideal, wrffdda_t_old_height, 'ght_agl', height_ideal, extrapolate = True)
