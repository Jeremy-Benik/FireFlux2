...
% Universal constants
% change the parameters here with the data in the excel file, and save the
% files as something different too so I know what's going on. I can even
% call it the row number so I know where I am. 
% change the constants how they are in the excel file and compare that to
% the data in the file. 
% If I do have questions about the parameters, ask Adam.
...
rho = 500;                      % Fuel density [kg/m3] THIS WAS HARDCODED
...
%this has been modified from the original. the original will be commented
%out
A = min(S/(2*pi), 1) * Chi_0 * DeltaH / (4 * q)
...
for i=1:maxit_R
    ...
    % This equation has been modified to the 2020 version
    R_b = min (2 * (s / 2 * pi) , 1) * (B*T_f^4)/(beta * rho_v * q)

    % compute rate of spread due to flame radiation (eq. 11)

    % This equation has also been modified from it's original. The original
    % will be commented out
    R_f = A * R * (1 + sin(gamma) - cos(gamma)) / (1 + (R * cos(gamma)) / (s * r_00))

    %R_f = A * R * (1 + sin(gamma) - cos(gamma)) / (1 + R * cos(gamma)...
    % / (s * r_00))

    % compute rate of spread due to convection
    R_c = (DeltaH * min(S, (2 * pi))/(q * tau_0 * beta) * ...
        ((fueldepthm/((2 * fueldepthm) + ...
        H_f) * tan(alpha)) + ((U/u_0) * exp(-K_drag * (beta ^ 1/2) * R))))  
