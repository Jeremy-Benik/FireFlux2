...
# Email Adam about the data and see if I need to run the for loops since it will take some computing power to do it.
# Assume he would say just interpolate to one column height
# Ask him what to do about the second domain. What should we fit into the second domain since they are different from the first domain
# What info should we incorporate into the wrffdda from the ideal case for the second domain.
#What should we fit into the second domain
#if he created the second domain, then I would need to crop the data for the d01 but I don't think that's how it is
#i could create a matric the size of do2 and fit the data inside of that from d01
#I would need to artifically generate those values
#Generate a matrix the size of the real d02 case, repeat the column 321 times and 201 times (or whatever the size is) [l, m, n]
#or np.repeat
#ask adam since the winds change too with location. 
#ask him what info should I put in d02 since the values change per location
#d02 would be extrapolation
#should we interpolate to one paticular point and repeat that for all locations?

#something else, pick a point, interpolate to that point and use that for all the area. 
#If he wants to replicate everything 
...
wrfin_ideal = nc.Dataset('/home/jbenik/FirefluxII/Codes_and_Data/Data/wrf_files/wrfinput_d02_ideal', 'r', format='NETCDF4') #ideal case. not more than one domain
wrfin_real = nc.Dataset('/home/jbenik/FirefluxII/Codes_and_Data/Data/wrf_files/wrfinput_d02_real', 'r', format='NETCDF4')
wrffdda = nc.Dataset('/home/jbenik/FirefluxII/Codes_and_Data/Data/wrf_files/wrffdda_d02', 'r', format='NETCDF4')
# %% wrfin variables
...
wrffdda_mod = nc.Dataset('/home/jbenik/FirefluxII/Codes_and_Data/Data/wrf_files/wrffdda_d02_mod', 'a', format='NETCDF4') #This is a copied file from the wrffdda and is the file that I will be writing my data to. 
