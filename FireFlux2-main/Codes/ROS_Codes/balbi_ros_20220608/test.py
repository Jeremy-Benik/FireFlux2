...
balbi_title = 'ROS       Category   Speed     Phi   fmc_g    u_0      T_f       T_a     n_iter'
balbi_fmt   = '{:.6f}   {:<8}   {:4.2f}    {:4.1f}    {:4.2f}   {:4.4f}    {:4.3f}   {:5.1f}  {:4d}'
print(balbi_title)
for p in ros_params:
    (category, speed, phi, fmc_g, T_a) = p[:]
    ros = ros_balbi(category, speed, tan(radians(phi)), fmc_g, T_a)
    print(balbi_fmt.format(ros.R, category, speed, phi, fmc_g, ros.u_0, ros.T_f, T_a, ros.n_iter))
