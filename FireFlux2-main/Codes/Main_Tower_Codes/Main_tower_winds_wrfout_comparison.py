...
out_path = 'main_tower_winds_wrfout_no_fire_atms_new_vars_1.pkl' #this is the no fire run, comment this out or the line above out
if not osp.exists(out_path):
    ...
    U_h_20 = wrf.interplevel(u_wrfout, ht, 20)[:, y_main, x_main] # Run this for all time
    ...
    ws_h_20_no_fire = np.sqrt((U_h_20 ** 2) + (V_h_20 ** 2))
    print("Interpolating W at 20 meters")
    W_h_20_no_fire = wrf.interplevel(w_wrfout, ht, 20)[:, y_main, x_main]
    ...
    ws_h_10_no_fire = np.sqrt((U_h_10 ** 2) + (V_h_10 ** 2))
    print("Interpolating W at 10 meters")
    W_h_10_no_fire = wrf.interplevel(w_wrfout, ht, 10)[:, y_main, x_main]
    ...
    ws_h_577_no_fire = np.sqrt((U_h_577 ** 2) + (V_h_577 ** 2))
    W_h_577_no_fire = wrf.interplevel(w_wrfout, ht, 5.77)[:, y_main, x_main]
    ...
    'ws_6':ws_6, 'uz20':uz20, 'uz10':uz10, 'uz6':uz6, 'wrfout_time_no_fire':wrfout_time_no_fire} #put the other variables in here such as wsw and ww
...
ax[1, 0].plot(wrfout_time, W_h_20[240:420], color = 'red', label = 'wrfout file')
#ax[2, 0].plot(np.array(wrfout.variables['XTIME'][:]), W_h_20.rolling(window = n).mean(), color = 'black', label = 'wrfout file')
...
ax[1, 1].plot(wrfout_time, W_h_10[240:420], color = 'red', label = 'wrfout file')
#ax[2, 1].plot(np.array(wrfout.variables['XTIME'][:]), W_h_10.rolling(window = n).mean(), color = 'black', label = 'wrfout file')
...
ax[1, 2].plot(wrfout_time, W_h_577[240:420], color = 'red', label = 'wrfout file')
#ax[2, 2].plot(np.array(wrfout.variables['XTIME'][:]), W_h_577.rolling(window = n).mean(), color = 'black', label = 'wrfout file')
