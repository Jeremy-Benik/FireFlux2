...
path = r'/home/jbenik/FireFlux2/Codes_and_Data/Data/Short_Tower_Data'
...
for filename in filenames:
    ...
    df = df1.truncate(before= np.where(df1['TIMESTAMP'] == '1/30/2013  15:00:00')[0][0], 
                        after=np.where(df1['TIMESTAMP'] == '1/30/2013  15:12:00')[0][0])
    time = df['TIMESTAMP']
    u = df['u']
    v = df['v']
    w = df['w']
    t = df['t']
    time2 = np.arange(0, len(time))
    fig, ax = plt.subplots(4, 1, figsize = (15, 10))
    plt.suptitle(f"Short tower plot for the {filename[-20]} Tower With 5 Second Averaging", fontsize = 18, fontweight = 'bold')
    ...
    ax[0].set_xlim(0, time2[-1])
    ax[0].set_xlabel('Time (Seconds)', fontsize = 16, fontweight = 'bold')
    ...
    ax[1].set_xlabel('Time (Seconds)', fontsize = 16, fontweight = 'bold')
    ...
    ax[2].set_xlim(0, time2[-1])
    ax[2].set_xlabel('Time (Seconds)', fontsize = 16, fontweight = 'bold')
    ...
    ax[3].set_xlim(0, time2[-1])
    ax[3].set_xlabel('Time (Seconds)', fontsize = 16, fontweight = 'bold')
    ...
    ax.set_ylabel('Time Averaged (Seconds)', fontsize = 16, fontweight = 'bold')
